const { fetchTempAcBal } = require('../services/tmsModulesService');

const getTempAcBal = async (req, res) => {
  try {
    ...
    const entries = await fetchTempAcBal(search, limit, {
      accountId,
      fromDate,
      toDate,
      type,
    ...
    console.error('GET /api/temp-acbal error:', error);
    res.status(500).json({ message: error.message });
  }
};

module.exports = { getTempAcBal };
