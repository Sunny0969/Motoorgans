...
const GroupAccountsLevels = () => {
  ...
  const loadAccountGroups = async () => {
    try {
      setLoading(true);
      const response = await api.get('/group-account-levels', {
        params: {
          parentGroup: filterParent !== 'All' ? filterParent : undefined,
  ...
  useEffect(() => {
    loadAccountGroups();
  }, [filterParent, filterLevel, searchTerm]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      if (editingId) {
        await api.put(`/group-account-levels/${editingId}`, formData);
        setEditingId(null);
      } else {
        await api.post('/group-account-levels', formData);
      }
      ...
      loadAccountGroups();
      alert(`Account group ${editingId ? 'updated' : 'created'} successfully!`);
    } catch (err) {
      setError(`Failed to ${editingId ? 'update' : 'create'} account group. Please try again.`);
      console.error('Error saving account group:', err);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this account group?')) {
      try {
        await api.delete(`/group-account-levels/${id}`);
        loadAccountGroups();
        alert('Account group deleted successfully!');
      } catch (err) {
        setError('Failed to delete account group. Please try again.');
          ...
          <div style={{
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                >
                  {saving ? 'Saving...' : (editingId ? 'Update' : 'Add Group')}
                </button>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                {loading ? 'Loading...' : `${accountGroups.length} groups`}
              </div>
            </div>
                  ...
                  <tbody>
                    {accountGroups.map((group, index) => (
                      <tr
                        key={group._id}
                        style={{
                          borderBottom: index === accountGroups.length - 1 ? 'none' : '1px solid #dee2e6'
                        }}
                      >
                        <td style={{
                          padding: '12px',
