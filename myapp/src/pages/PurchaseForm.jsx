...
function PurchaseForm() {
  ...
  const applyPurchase = useCallback((p) => {
    if (!p) return;
    setHeader({
  ...
  const loadLatest = useCallback(async () => {
    ...
    setMessage(null);
    try {
      const res = await api.get('/purchases/latest');
      applyPurchase(res.data);
    } catch (err) {
  ...
  const loadSupplierBalance = async (supplierId) => {
    ...
    try {
      const res = await api.get(`/purchases/supplier/${supplierId}/balance`);
  ...
  const handleProductSelect = (product) => {
    ...
    loadProductInfo(pid, entry.locationId);
  };
  ...
  const handleAddLine = () => {
    ...
    const newLine = { sr: lines.length + 1, ...buildLineFromEntry() };
  ...
  const handleRemoveLine = () => {
    ...
    const filtered = lines.filter((_, i) => i !== editIndex).map((l, i) => ({ ...l, sr: i + 1 }));
    setLines(filtered);
    resetEntry();
  };
  ...
  const handleSave = async () => {
    ...
    setMessage(null);
    try {
      const res = await api.post('/purchases', buildPayload());
      setMessage(res.data.message || 'Purchase saved.');
      applyPurchase(res.data);
  ...
  const handleUpdateRecord = async () => {
    if (!savedDoc) {
      alert('No saved purchase to update.');
      return;
    }
    if (!window.confirm(`Update purchase #${savedDoc}?`)) return;
    setLoading(true);
    try {
      const res = await api.put(`/purchases/${savedDoc}`, buildPayload());
  ...
  const handleDelete = async () => {
    if (!savedDoc) return;
    if (!window.confirm(`Delete purchase #${savedDoc}?`)) return;
    setLoading(true);
    try {
      await api.delete(`/purchases/${savedDoc}`);
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(`/purchases/search/${encodeURIComponent(q)}`);
      if (!res.data.length) {
        alert('No purchase found.');
        return;
      }
      applyPurchase(res.data[0]);
    } catch (err) {
  ...
  const handlePrintBill = () => {
    const rows = lines.map((l) => `
      <tr>
        ...
        <td>${l.productName}</td>
        <td>${l.uom}</td>
        <td>${formatPackingCell(l)}</td>
        <td align="right">${l.pcs}</td>
        <td align="right">${l.rate}</td>
        <td align="right">${l.amount}</td>
        <td align="right">${l.net}</td>
      </tr>`).join('');
    const html = `<!DOCTYPE html><html><head><title>Purchase ${header.invoiceNo}</title>
      ...
      <table><thead><tr><th>Sr</th><th>Product</th><th>UOM</th><th>Packing</th><th>Pcs</th><th>Rate</th><th>Amount</th><th>Net</th></tr></thead>
      <tbody>${rows}</tbody></table>
      <p style="text-align:right"><b>Bill Amount:</b> ${footerTotals.billAmount} &nbsp;
      <b>Net Payable:</b> ${footerTotals.netPayable}</p></body></html>`;
      ...
      <div style={{ padding: '8px' }}>
        ...
        <div style={{ backgroundColor: '#e8e8e8', border: '1px solid #888', padding: '8px', marginBottom: '8px' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'flex-end' }}>
            ...
            <div>
              <button type="button" onClick={handleSearch} style={{ padding: '5px 10px', fontSize: '12px', marginTop: '16px' }}>Search Purchases</button>
            </div>
            ...
            <div>
              <span style={lbl}>Select Supplier</span>
              <div style={{ display: 'flex', gap: '4px' }}>
                <input value={header.supplierCode} readOnly style={{ ...inp, width: '50px' }} placeholder="Code" />
                <select value={header.supplierId} onChange={handleSupplierDropdown} style={{ ...inp, width: '200px' }}>
                ...
                <button type="button" onClick={() => setShowSupplierModal(true)} style={{ padding: '4px 8px' }}>…</button>
              </div>
            </div>
            <div>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #888', padding: '8px', marginBottom: '8px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '6px', marginBottom: '6px' }}>
            ...
            <div><span style={lbl}>Remarks</span><input name="remarks" value={entry.remarks} onChange={handleEntryChange} style={inp} /></div>
            <div>
            ...
            <div><span style={lbl}>Builty</span><input name="builty" value={entry.builty} onChange={handleEntryChange} style={inp} /></div>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #888', padding: '6px', marginBottom: '8px' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
              <thead>
                <tr style={{ backgroundColor: '#e0e0e0' }}>
                  {['Sr.#', 'Product', 'U.O.M', 'Packing', 'Pc(s)', 'Rate', 'Amount', 'Disc.%', 'Discount', 'Net', 'SchPc'].map((h) => (
                ...
                {lines.length === 0 ? (
                  <tr>
                    {Array(11).fill(0).map((_, i) => (
                      <td key={i} style={{ border: '1px solid #999', padding: '3px', textAlign: 'center' }}>0</td>
                    ))}
                  </tr>
                ) : lines.map((line, idx) => (
                  <tr
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
          ...
          <div style={{ width: '320px', backgroundColor: '#fff', border: '1px solid #888', padding: '8px' }}>
            {[
              ...
              ['Net Payable', footerTotals.netPayable, true],
        ...
        {/* Action bar */}
        <div style={{ backgroundColor: '#ff8c00', padding: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            ...
            {btn('❌ Delete', handleDelete, !savedDoc, '#ffb6b6')}
