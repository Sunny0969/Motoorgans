...
const API = '/claims-in-from-customer';

function ClaimInFromCustomer() {
  ...
  const loadCustomers = useCallback(async () => {
    try {
      ...
      try {
        ...
        setCustomers(list.filter((a) => a.ACType === 5 || a.acType === 5));
      } catch {
        setCustomers([]);
      }
    }
  ...
  const applyVoucher = useCallback((p) => {
    ...
    setLines(
      (p.products || []).map((row, i) => ({
        ...
        net: row.net ?? row.netAmount ?? '0.00',
        uom: row.uom || 'PCS',
      })),
    );
    resetEntry();
    ...
    try {
      ...
      setEntry((prev) => {
        const updated = {
          ...
          rate: p.saleRate != null ? String(p.saleRate) : (p.purchaseRate != null ? String(p.purchaseRate) : prev.rate),
  ...
  const handleUpdateRecord = async () => {
    if (!savedDoc) return;
    if (!window.confirm(`Update claim in #${savedDoc}?`)) return;
    setLoading(true);
    try {
  ...
  const handleDelete = async () => {
    if (!savedDoc) return;
    if (!window.confirm(`Delete claim in #${savedDoc}?`)) return;
    setLoading(true);
    try {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!res.data.length) {
        alert('No claim in voucher found.');
        return;
      }
  ...
  const handlePrint = () => {
    ...
    const html = `<!DOCTYPE html><html><head><title>Claim In ${header.invoiceNo}</title>
      ...
      <p><b>Buyer:</b> ${header.customerCode} - ${header.customerName} &nbsp; <b>Payment:</b> ${header.paymentType}</p>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'flex-end' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '4px' }}>
                <select value={header.customerId} onChange={handleCustomerDropdown} style={{ ...inp, width: '220px' }}>
...
export default ClaimInFromCustomer;
