...
function PurchaseReturn() {
  ...
  const footerTotals = useMemo(() => {
    ...
    const cashReceived = parseFloat(header.cashReceived) || 0;
    const netPayable = billAmount + prevBal - cashReceived;
    return {
  ...
  }, [lineTotals, header.discPercentFooter, header.previousBalance, header.cashReceived]);
  ...
  const applyVoucher = useCallback((p) => {
    ...
    setHeader({
      ...
      cashReceived: p.cashReceived != null ? String(p.cashReceived) : (p.cashPaid != null ? String(p.cashPaid) : ''),
  ...
  const buildPayload = () => ({
    ...
    cashReceived: header.cashReceived,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      applyVoucher(res.data);
      if (window.confirm('Print return voucher?')) handlePrintBill();
  ...
  const handleUpdateRecord = async () => {
    if (!savedDoc) return;
    if (!window.confirm(`Update purchase return #${savedDoc}?`)) return;
    setLoading(true);
    try {
  ...
  const handleDelete = async () => {
    if (!savedDoc) return;
    if (!window.confirm(`Delete purchase return #${savedDoc}?`)) return;
    setLoading(true);
    try {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!res.data.length) {
        alert('No purchase return found.');
        return;
      }
  ...
  const handleNew = async () => {
    await loadNextInvoice();
    setMessage('New purchase return.');
  };
  ...
  const handlePrintBill = () => {
    const rows = lines.map((l) => `
      <tr>
        ...
        <td align="right">${l.rate}</td><td align="right">${l.amount}</td><td align="right">${l.net}</td>
      </tr>`).join('');
    const html = `<!DOCTYPE html><html><head><title>Purchase Return ${header.invoiceNo}</title>
      ...
      <p><b>Supplier:</b> ${header.supplierCode} - ${header.supplierName}</p>
    ...
    <div style={{ backgroundColor: '#d4d4d4', minHeight: '100vh', fontFamily: 'Tahoma, Arial, sans-serif' }}>
      ...
      <div style={{ padding: '8px' }}>
        ...
        <div style={{ backgroundColor: '#fff', border: '1px solid #888', padding: '8px', marginBottom: '8px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(11, 1fr)', gap: '6px', marginBottom: '6px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
          ...
          <div style={{ width: '320px', backgroundColor: '#fff', border: '1px solid #888', padding: '8px' }}>
            {[
              ...
              ['Cash Received', header.cashReceived, false, 'cashReceived'],
