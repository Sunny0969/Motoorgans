...
const OpeningStock = () => {
  ...
  const loadNextInvoice = useCallback(async () => {
    try {
      const res = await api.get('/opening-stock/next-number');
  ...
  const loadLatest = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/opening-stock/latest');
  ...
  const loadProductInfo = async (productId) => {
    if (!productId) return;
    try {
      const res = await api.get(`/opening-stock/product/${productId}`);
  ...
  const loadByInvoice = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get(`/opening-stock/${inv}`);
      applyInvoice(res.data.data);
      setMessage({ type: 'success', text: `Loaded invoice #${inv}.` });
    } catch (err) {
  ...
  const saveInvoice = async (isUpdate) => {
    ...
    try {
      ...
      const res = savedDoc && isUpdate
        ? await api.put(`/opening-stock/${savedDoc}`, payload)
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm(`Delete opening stock #${savedDoc}?`)) return;
    setLoading(true);
    try {
      await api.delete(`/opening-stock/${savedDoc}`);
      setMessage({ type: 'success', text: `Deleted invoice #${savedDoc}.` });
      setLines([]);
      setSavedDoc(null);
      await loadNextInvoice();
    } catch (err) {
  ...
  return (
    <div style={s.page}>
      <div style={s.titleBar}>Opening Stock</div>
      ...
      <div style={s.gridWrap}>
        <table style={s.table}>
          ...
          <tbody>
            {lines.length === 0 ? (
              <tr><td colSpan={6} style={s.emptyCell}>—</td></tr>
            ) : (
              lines.map((row, idx) => (
                <tr
...
const s = {
  ...
  selProduct: { minWidth: 200, maxWidth: 320, padding: '2px 4px', border: '1px solid #888' },
  selLoc: { minWidth: 100, padding: '2px 4px', border: '1px solid #888' },
  ...
  gridWrap: { border: '1px solid #888', marginBottom: 6, maxHeight: 320, overflow: 'auto' },
